...
def load_label_data(filename):
    ...
    with open(filename) as infile:
        ...
        for line in infile:
            ...
            obj_type = fields[
                0].lower()  # object type [car, pedestrian, cyclist, ...]
            truncation = float(fields[1])  # truncation [0..1]
            occlusion = int(float(fields[2]))  # occlusion  [0,1,2]
            obs_angle = float(fields[3])  # observation angle [rad]
            x1 = int(float(fields[4]))  # left   [px]
            y1 = int(float(fields[5]))  # top    [px]
            x2 = int(float(fields[6]))  # right  [px]
            y2 = int(float(fields[7]))  # bottom [px]
            h = float(fields[8])  # height [m]
            w = float(fields[9])  # width  [m]
            l = float(fields[10])  # length [m]
            X = float(fields[11])  # X [m]
            Y = float(fields[12])  # Y [m]
            Z = float(fields[13])  # Z [m]
            yaw = float(fields[14])  # yaw angle [rad]
            if len(fields) >= 16:
                score = float(fields[15])  # detection score
            else:
                score = 1
            detect_id = index
            t_data = [obj_type, truncation, occlusion, obs_angle, x1, y1, x2, y2, h, w, l, X, Y, Z, yaw, score]
            data.append(t_data)
            index = index + 1
    return data
