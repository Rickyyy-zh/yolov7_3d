...
def create_dataloader_Rope3D(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = Rope3dDataSet(path, imgsz, batch_size,
                                      augment=augment,  # augment images
                                      hyp=hyp,  # augmentation hyperparameters
                                      rect=rect,  # rectangular training
                                      cache_images=cache,
    ...
    dataloader = loader(dataset,
                        ...
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=Rope3dDataSet.collate_fn4 if quad else Rope3dDataSet.collate_fn)
    return dataloader, dataset
...
class Rope3dDataSet(Dataset):  # for training/testing
    def __init__(self, path, img_size=[1920,1080], batch_size=32, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
        self.hyp = hyp
        ...
        self.path = path       
        #self.albumentations = Albumentations() if augment else None
        self.ImageSet_path = path + prefix + ".txt"
        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:
                p = Path(p)  # os-agnostic
        ...
        for i, (im_file, lb_file, ext_file, calib_file) in enumerate(pbar):
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    with open(lb_file, 'r') as f:
                        l = []
                        for x_ in f.read().strip().splitlines():
                            x_ = x_.split()
                            if x_[0] in CLASS_.keys():
                                x_[0] = CLASS_[x_[0]]
                            else:
                                continue
                    ...
                    if len(l):
                        # assert (l >= 0).all(), 'negative labels'
                        assert (l[:, 4:8] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                        assert np.unique(l, axis=0).shape[0] == l.shape[0], 'duplicate labels'
                    else:
                        ne += 1  # label empty
    ...
    def __getitem__(self, index):
        ...
        nL = len(labels)  # number of labels
        if nL:
            labels[:, 4:8] = xyxy2xywh(labels[:, 4:8])  # convert xyxy to xywh
            labels[:, [5, 7]] /= img.shape[0]  # normalized height 0-1
            labels[:, [4, 6]] /= img.shape[1]  # normalized width 0-1
            # labels[:, 0] = CLASS_[labels[:, 0]]
...
def random_perspective_3D(img, targets=(), p2 = np.zeros([4, 4], dtype=float), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            xy = np.ones((n * 4, 3))
            # xy[:, :2] = targets[:, [1, 2, 3, 4, 1, 4, 3, 2]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy[:, :2] = targets[:, [4, 5, 6, 7, 4, 7, 6, 5]].reshape(n * 4, 2)  # x1y1, x2y2, x1y2, x2y1
            xy = xy @ M.T  # transform
            xy = (xy[:, :2] / xy[:, 2:3] if perspective else xy[:, :2]).reshape(n, 8)  # perspective rescale or affine
